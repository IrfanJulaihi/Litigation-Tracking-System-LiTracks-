...
$_SESSION["SearchItem"]=$_POST['category'] ;
mysql_select_db($database_localhost, $localhost);
...
$totalRows_searching = $_POST['category'];
}
$colname_searching = "-1";
if (isset($_POST['category'])) {
$colname_searching =$_POST['category'] ;
}
mysql_select_db($database_localhost, $localhost);


$query_searching = sprintf("SELECT * FROM information WHERE Category LIKE %s ORDER BY id DESC", GetSQLValueString("%" . $colname_searching . "%", "text"));
$searching = mysql_query($query_searching, $localhost) or die(mysql_error());
$row_searching = mysql_fetch_assoc($searching);
...
$_SESSION['FindCategory']=$_POST['category'];
?>
...
<body>
<h2>
  <form id="form1" name="form1" method="post" action="PrintAllSearchCategory.php">
