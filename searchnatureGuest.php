...
$query_searchNature = sprintf("SELECT * FROM information WHERE nature LIKE %s and Category LIKE %s ORDER BY ID DESC", GetSQLValueString("%" . $colname_searchNature . "%", "text"),GetSQLValueString("%" . $_SESSION['DIVISION'] . "%", "text"));
$searchNature = mysql_query($query_searchNature, $localhost) or die(mysql_error());
$row_searchNature = mysql_fetch_assoc($searchNature);
$totalRows_searchNature = mysql_num_rows($searchNature);}
...
$_SESSION['FindNatureGuest']=$_POST['findnature'];
?>
...
<body>
<h2>
  <form id="form1" name="form1" method="post" action="PrintAllSearchNatureGuest.php">
