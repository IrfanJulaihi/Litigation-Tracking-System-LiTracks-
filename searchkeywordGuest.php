...
$colname_searching =$_POST['findKeyword'] ;
}
mysql_select_db($database_localhost, $localhost);
$Legal='LAD';
if($_SESSION['DIVISION']==$Legal){
	$query_searching = sprintf("SELECT * FROM information WHERE Keyword LIKE %s ORDER BY ID DESC", GetSQLValueString("%" . $colname_searching . "%", "text"));
...
$query_searching = sprintf("SELECT * FROM information WHERE Keyword LIKE %s and Category LIKE %s ORDER BY ID DESC", 
GetSQLValueString("%" . $colname_searching . "%", "text"),GetSQLValueString("%" . $_SESSION['DIVISION'] . "%", "text"));
$searching = mysql_query($query_searching, $localhost) or die(mysql_error());
$row_searching = mysql_fetch_assoc($searching);
$totalRows_searching = mysql_num_rows($searching);
}
...
$_SESSION['FindKeywordGuest']=$_POST['findKeyword'];
?>
...
<h2>
  <form id="form1" name="form1" method="post" action="PrintAllSearchKeywordGuest.php">
  <p id="PrintTitle">Please enter report header / title(e.g. &quot;Report as at 31 December 2015&amp;&quot;</p>
  <p>
