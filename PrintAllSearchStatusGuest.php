...
$query_Recordset1 = sprintf("SELECT * FROM information WHERE status LIKE %s ORDER BY ID DESC", 
GetSQLValueString("%" .$_SESSION['FindStatusGuest'] . "%", "text"));
$Recordset1 = mysql_query($query_Recordset1, $localhost) or die(mysql_error());
$row_Recordset1 = mysql_fetch_assoc($Recordset1);
$totalRows_Recordset1 = mysql_num_rows($Recordset1);
}else{
mysql_select_db($database_localhost, $localhost);
$query_Recordset1 = sprintf("SELECT * FROM information WHERE status LIKE %s and Category Like %s ORDER BY ID DESC", GetSQLValueString("%" .$_SESSION['FindStatusGuest'] . "%", "text"),
